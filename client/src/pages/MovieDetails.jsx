  ...
  return show ? (
    <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
        ...
        <div className="flex items-center gap-4 w-max px-4">
          {show.movie.casts.slice(0,12).map((cast,index) => (
            <div key={index} className="flex flex-col items-center text-center">
              <img src={image_base_url + cast.profile_path} alt={`${cast.name}`} className="rounded-full h-16 md:h-16 aspect-square object-cover" />
      ...
      <div className="flex justify-center mt-20">
          <button onClick={() => {navigate('/movies'); scrollTo(0,0)}} className="px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-md cursor-pointer">Show more</button>
      </div>
    </div>
  )
  : 
